            ...
            try:
                with contextlib.closing(wave.open(fname,'r')) as f:
                    ...
                    wav_files.append(fname)
                        ...
                        corrupted+=1
    return total, files, corrupted,wav_files

def extract_non_corrupted_files(path="/Users/mandeebot/Desktop/proj_data/Berom_ASR/Berom_Speech_Dataset/wav"):
    ...
    for directory in os.listdir(path): # parent directory of the recordings, it should contain folders that contain wav,json and txt files
        if os.path.isdir("./wav/"+directory):
            linker = []
            to_remove = []
            linker_file = [file for file in os.listdir("./wav/"+directory) if file.endswith(".txt")][0]
            linker_data = open("./wav/"+directory+"/"+linker_file).readlines()
            for file in os.listdir("./wav/"+directory):                
                if file.endswith(".wav"):
                    fname = "./wav/"+directory+"/"+file
                    try:
                        with contextlib.closing(wave.open(fname,'r')) as f:
                            frames = f.getnframes()
                            rate = f.getframerate()
